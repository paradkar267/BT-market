...
export default function ProductPage() {
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-500 ${isDark ? 'bg-[#090A0F] text-white' : 'bg-gray-50/70 text-black'}`}>
      ...
      {/* Top Sticky Header */}
      <nav className="sticky top-0 z-50 bg-white/80 dark:bg-[#090A0F]/80 backdrop-blur-xl border-b border-black/[0.06] dark:border-white/10 transition-colors">
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 md:px-12 h-20 flex items-center justify-between">
          <Logo />
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            <button 
              ...
              className="hidden sm:flex items-center gap-2 text-xs font-semibold text-gray-500 hover:text-black dark:text-gray-400 dark:hover:text-white bg-gray-100 dark:bg-white/5 hover:bg-gray-200/70 dark:hover:bg-white/10 px-3.5 py-2 rounded-full border border-black/5 dark:border-white/10 transition-all cursor-pointer"
            >
              <Search className="w-3.5 h-3.5" />
              <span>Search templates</span>
              <kbd className="px-1.5 py-0.5 rounded bg-white dark:bg-black/50 border border-black/10 dark:border-white/10 text-[10px] text-gray-400">Ctrl+K</kbd>
            </button>

            <button 
              onClick={() => requireAuth(() => navigate('/cart'))} 
              className="relative p-2.5 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full transition-colors cursor-pointer"
              title="Shopping Cart"
            >
              <ShoppingCart className="w-5 h-5" />
              {cartItems.length > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 bg-black text-white dark:bg-white dark:text-black text-[11px] font-extrabold rounded-full flex items-center justify-center shadow-md animate-scaleIn">
                  {cartItems.length}
                </span>
              )}
            </button>
