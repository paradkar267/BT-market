    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[999999] w-screen h-screen min-h-screen bg-white dark:bg-slate-950 flex flex-col overflow-hidden text-slate-900 dark:text-white select-none"
        >
          {/* Ambient Lighting Gradient Beams */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-[300px] bg-gradient-to-b from-amber-500/10 via-amber-500/5 to-transparent blur-3xl pointer-events-none -z-10" />

          {/* Top Fullscreen Header */}
          <header className="w-full border-b border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl shrink-0">
            <div className="w-full px-4 sm:px-8 lg:px-14 h-16 sm:h-18 flex items-center justify-between">
              {/* Brand & Explorer Badge */}
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-amber-400 via-amber-500 to-yellow-500 flex items-center justify-center text-slate-950 shadow-sm shadow-amber-500/30">
                  <Compass className="w-5 h-5 text-slate-950" />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-sm sm:text-base font-black tracking-tight text-slate-950 dark:text-white leading-none">
                      BIZLEAP SEARCH
                    </h2>
                    <span className="px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-widest text-amber-700 dark:text-amber-300 bg-amber-100/80 dark:bg-amber-950/70 border border-amber-300/70 dark:border-amber-800/70 rounded-md">
                      FULLSCREEN
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs sm:text-sm font-bold text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 transition-all cursor-pointer select-none group shadow-xs"
                  aria-label="Close search"
                >
                  <span className="text-[11px] font-mono font-bold uppercase text-slate-400 group-hover:text-slate-700 dark:group-hover:text-white">
                    ESC
                  </span>
          ...
          <div className="w-full border-b border-slate-200/80 dark:border-slate-800/80 bg-slate-50/60 dark:bg-slate-900/40 shrink-0">
            <div className="w-full px-4 sm:px-8 lg:px-14 py-5 sm:py-6">
              {/* Full Width Search Input */}
              <div className="relative flex items-center gap-4 bg-white dark:bg-slate-900 px-5 sm:px-7 py-4 sm:py-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-[0_10px_30px_rgba(0,0,0,0.06)] focus-within:ring-4 focus-within:ring-amber-500/15 focus-within:border-amber-500/60 transition-all">
                ...
                <input
                  ...
                  className="w-full bg-transparent text-lg sm:text-2xl lg:text-3xl font-semibold text-slate-900 dark:text-white placeholder:text-slate-400 placeholder:font-normal outline-none"
                />
                {query && (
                  <button
                    type="button"
                ...
                <span className="hidden sm:inline-flex items-center px-3 py-1 text-xs font-mono font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shrink-0">
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                  ...
                  {TRENDING_SEARCHES.map((term) => (
                    <button
                      ...
                      className="px-2.5 py-1 text-xs font-semibold rounded-lg bg-white dark:bg-slate-800/80 hover:bg-amber-50 dark:hover:bg-amber-950/40 hover:text-amber-600 dark:hover:text-amber-400 border border-slate-200/80 dark:border-slate-800 transition-colors shrink-0 cursor-pointer"
                    >
                      {term}
                    </button>
                  ))}
          ...
          >
            ...
            <div className="flex items-center justify-between mb-6 pb-3 border-b border-slate-200/60 dark:border-slate-850">
              ...
              <span className="text-xs text-slate-400 font-mono hidden sm:inline-flex items-center gap-1.5">
                ...
                <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[10px]">↑</kbd>
                <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[10px]">↓</kbd>
                <span>and</span>
                <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[10px]">Enter</kbd>
                <span>to view</span>
              </span>
            </div>
          ...
          {/* Full-Width Footer Status & Keyboard Navigation Bar */}
          <footer className="w-full border-t border-slate-200/80 dark:border-slate-800 bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl shrink-0">
            <div className="w-full px-4 sm:px-8 lg:px-14 h-12 flex items-center justify-between text-xs text-slate-400">
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1.5 font-medium">
                  <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-mono text-[10px] text-slate-600 dark:text-slate-300">↑</kbd>
                  <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-mono text-[10px] text-slate-600 dark:text-slate-300">↓</kbd>
                  navigate
                </span>
                <span className="flex items-center gap-1.5 font-medium">
                  <kbd className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-mono text-[10px] text-slate-600 dark:text-slate-300">ENTER</kbd>
                  to open
                </span>
                <span className="flex items-center gap-1.5 font-medium">
                  <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-mono text-[10px] text-slate-600 dark:text-slate-300">ESC</kbd>
                  to close
                </span>
              </div>
