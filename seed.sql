INSERT INTO public.templates (id, tag, category, title, author, sales, rating, price, image, description, keywords) VALUES
...
CREATE TABLE public.purchases (
  ...
  template_id int REFERENCES public.templates NOT NULL,
  created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- Turn on the firewall (Row Level Security)
...
-- Policy: Users can only CREATE their own purchases
...
-- Sample Starter Promo Codes
INSERT INTO public.coupons (code, discount_type, discount_value, min_order_amount, usage_limit, times_used, expires_at, is_active) VALUES
('LAUNCH50', 'percentage', 50, 2000, 100, 14, timezone('utc'::text, now() + interval '30 days'), true),
('WELCOME20', 'percentage', 20, 1000, 500, 68, timezone('utc'::text, now() + interval '60 days'), true),
('FLAT500', 'flat', 500, 3000, 200, 32, timezone('utc'::text, now() + interval '45 days'), true),
('BIZLEAP1000', 'flat', 1000, 5000, 50, 9, timezone('utc'::text, now() + interval '15 days'), true)
ON CONFLICT (code) DO NOTHING;

-- ==========================================
-- Coupon Redemptions (1-per-user tracking)
-- ==========================================
...
CREATE POLICY "Allow insert on coupon redemptions"
...
CREATE TABLE IF NOT EXISTS public.campaigns (
  ...
  preview_text text,
  type text NOT NULL DEFAULT 'announcement', -- 'launch', 'sale', 'announcement', 'vip'
  ...
  template_id text,
  coupon_code text,
  audience_type text NOT NULL DEFAULT 'all', -- 'all', 'category', 'template_buyers', 'test'
